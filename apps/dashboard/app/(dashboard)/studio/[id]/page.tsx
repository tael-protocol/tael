...
import { getProduct, listActions, listContent } from "../../../../features/products/queries";
...
export default async function StudioDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!product) notFound();
  const [content, actions] = await Promise.all([listContent(id), listActions(id)]);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <StudioTabs product={product} content={content} actions={actions} />
    </div>
  );
}
